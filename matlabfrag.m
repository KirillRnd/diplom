...
%  1. Does not use font size if it coincides with the "Default" size
%  2. Add "\MATLABFont" before \selectfont for all the text. This 
%     allows to use some font (in LaTeX), different from the
%     document's main font. By default, it is empty;
%     use as e.g. \providecommand{\MATLABFont}{\sffamily\sansmath\small}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
try
  ...
  for ii=1:size(PsfragCmds,1)
    ...
    if NewFontStyle
      ...
      qq=get(0,'DefaultAxesFontSize');
      % if the current font is the Default one, do not set it
      % explicitely, let LaTeX to do so
      if (CurrentFontSize~=qq)
        fprintf(fid,['\n%%\n\\providecommand\\%s%s{\\color[rgb]{%.3f,%.3f,'...
          '%.3f}\\fontsize{%d}{%d}%s%s%s\\MATLABFont\\selectfont\\strut}%%'],FontStylePrefix,...
           char(FontStyleId),CurrentColour(1),CurrentColour(2),...
          CurrentColour(3),CurrentFontSize,CurrentFontSize,Angle,Weight,Fixed);
      else
         fprintf(fid,['\n%%\n\\providecommand\\%s%s{\\color[rgb]{%.3f,%.3f,'...
          '%.3f}%s%s%s\\MATLABFont\\selectfont\\strut}%%'],FontStylePrefix,...
           char(FontStyleId),CurrentColour(1),CurrentColour(2),...
          CurrentColour(3),Angle,Weight,Fixed);
      end
      NewFontStyle = 0;
    end
